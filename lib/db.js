...
async function getDB() {
  ...
  // Cupom interno de primeira compra: gerado sozinho, funciona pra sempre,
  // sem limite total de usos — mas só 1 vez por comprador (ver cupomValidoParaUsuario).
  if (!data.cupons.find(function(c) { return c.codigo === 'PRIMEIRACOMPRA20%'; })) {
    data.cupons.push({
      codigo: 'PRIMEIRACOMPRA20%',
      tipo: 'porcentagem',
      valor: 20,
      limite: null,
      usos: 0,
      ativo: true,
      unicoPorComprador: true,
      usados_por: [],
      interno: true // marcador — não deveria ser deletado pelo admin por engano
    });
  }
...
function criarSolicitacoesVipClan(data, pedido, produtos) {
  ...
  pedido.itens.forEach(function(item) {
    ...
    const vipVinculado = produto.vipClanVinculadoId ? produtos.find(function(p) { return p.id === produto.vipClanVinculadoId; }) : null;
    ...
    data.solicitacoes_vip_clan.push({
      ...
      vip_vinculado_cargo_id: vipVinculado ? vipVinculado.cargoVipId : null,
      vip_vinculado_nome: vipVinculado ? vipVinculado.nome : null,
...
// Verifica se um cupom pode ser usado por essa pessoa: precisa estar ativo,
// ainda não ter estourado o limite total (se tiver limite) e, se for "único
// por comprador", essa pessoa ainda não ter usado ele antes.
function cupomValidoParaUsuario(cupom, discordId) {
  if (!cupom || !cupom.ativo) return false;
  if (cupom.unicoPorComprador) {
    if (cupom.usados_por && cupom.usados_por.indexOf(discordId) !== -1) return false;
  } else if (cupom.limite != null && cupom.usos >= cupom.limite) {
    return false;
  }
  return true;
}

// Marca o cupom como usado por essa pessoa (chamado quando o pagamento é confirmado)
function marcarCupomUsado(data, codigoCupom, discordId) {
  const cupom = data.cupons.find(function(c) { return c.codigo === codigoCupom; });
  if (!cupom) return;
  cupom.usos = (cupom.usos || 0) + 1;
  if (cupom.unicoPorComprador) {
    cupom.usados_por = cupom.usados_por || [];
    if (cupom.usados_por.indexOf(discordId) === -1) cupom.usados_por.push(discordId);
  }
}

module.exports = { getDB, saveDB, registrarLog, notificarDiscord, ehAdmin, atualizarDoador, criarSolicitacoesVipClan, cupomValidoParaUsuario, marcarCupomUsado };
